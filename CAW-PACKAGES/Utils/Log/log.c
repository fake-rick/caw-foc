...
void CAW_LOG_Write(const char* fmt, CAW_LOG_LEVEL level, const char* file,
  ...
  if (level == LOG_LEVEL_DEBUG) {
    ...
    snprintf(buf, sizeof(buf),
             "\033[0;36mCawFOC-[DEBUG] <%s> | <%d> | <%s>: %s\033[m\r\n", file,
             line, func, tmp);
#else
...
#ifdef CAWFOC_LOG_COLOR_ENABLE
    snprintf(buf, sizeof(buf),
             "\033[0;32mCawFOC-[INFO] <%s> | <%d> | <%s>: %s\033[m\r\n", file,
             line, func, tmp);
#else
...
#ifdef CAWFOC_LOG_COLOR_ENABLE
    snprintf(buf, sizeof(buf),
             "\033[0;33mCawFOC-[WARN] <%s> | <%d> | <%s>: %s\033[m\r\n", file,
             line, func, tmp);
#else
...
#ifdef CAWFOC_LOG_COLOR_ENABLE
    snprintf(buf, sizeof(buf),
             "\033[0;31mCawFOC-[ERROR] <%s> | <%d> | <%s>: %s\033[m\r\n", file,
             line, func, tmp);
#else
...
#ifdef CAWFOC_LOG_COLOR_ENABLE
      snprintf(buf, sizeof(buf),
               "\033[0;35mCawFOC-[FAIL] <%s> | <%d> | <%s>: %s\033[m\r\n", file,
               line, func, tmp);
#else
      snprintf(buf, sizeof(buf), "CawFOC-[ERROR] <%s> | <%d> | <%s>: %s\r\n",
               file, line, func, tmp);
...
void CAW_LOG_Welcome(void) {
  char buf[] =
      "\r\n"
      "|----------------------------------------|\r\n"
      "|            \033[0;31mWelcome to CawFOC\033[m           |\r\n"
      "|----------------------------------------|\r\n"
      "|            Author: FakeRick            |\r\n"
      "|----------------------------------------|\r\n"
      "|              MIT License               |\r\n"
